...
////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////
# define FOR_THE_HORDE auto t = loads<u32>(); while( t-- )
...
struct Algorithm {
  ...
  // this is kind of a modification of the canonical binary
  // lifting i guess but is kind of a messy so for every node u
  // there is a list of parents p[u] such that p[u][j] is the
  // 2^j-th parent
  //
  void levlup( i32 a ){
    i32 j = 0;
  ...
  void bld() {
    ...
    p.resize( n + 1 );
    for( i32 j = 2; j <= n; ++j ){
      p[ j ].push_back( loads<i32>());
      c[ p[ j ].back()].push_back( j );
    }
    dbg( c ); dbg( p );
    dfs();
    dbg( p );
  ...
  // 2^j + r = k
  // k = 20 -> j=4,r=4
  static paint_t f( i32 k ){
    i32 j = 31 - __builtin_clz( k );
    i32 r = k - (1 << j);
    return {j,r};
  }

# define T true
  i32 query( i32 x, i32 k ){
    while( T ){
      auto [j,r] = f( k );
      dbg2( j, r );
      if( j >= siz( p[x] )){ return -1; }
      if( r == 0 ){
        return p[x][j];
      }
      x = p[x][j];
...
// Memory limit: 512 MB
// 
// A company has n employees, who form a tree hierarchy where 
// each employee has a boss, except for the general director.
// Your task is to process q queries of the form: who is 
// employee x's boss k levels higher up in the hierarchy?
//
// [ Input ]
// The first input line has two integers n and q: the number of
// employees and queries. The employees are numbered 1,2,...,n,
// and employee 1 is the general director.
//
// The next line has n-1 integers e_2,e_3,...,e_n: for each
// employee 2,3,...,n their boss. Finally, there are q lines
// describing the queries. Each line has two integers x and k:
// who is employee x's boss k levels higher up? 
//
// [ Output ]
// Print the answer for each query. If such a boss does not
// exist, print -1.
//
// [ Constraints ]
// 1 <= n,q <= 2e5
