...
class Algorithm {
public:
    Algorithm( i64 n, i64 m ):
        n( n ), m( m )
    {}
    void pusk() {
        if( n < m ){ swap( n, m ); }
        const auto varionts = (1 << m);
        w<i64> matches( varionts );
        For( top, 0, varionts ){
            For( bot, 0, varionts ){
                if( match( top, bot )){
                    matches[top].psh( bot );
        ...
        auto f = w<i64>( n + 1, vi64( varionts )); f[0][0] = 1;
        w<i64> aux( n ); aux[0].psh( 0 );
        For( i, 1, n ){
            for( auto top: aux[i - 1] ){
                for( auto bot: matches[ top ]){
                    if( !f[i][bot] ){
                        aux[i].psh( bot );
                    }
                    f[i][bot] += f[i - 1][top];
        ...
        for( auto top: aux[n - 1] ){
            if( !f[n - 1][top] ){ continue; }
            if( match( top, 0 )){
                f[n][0] += f[n - 1][top];
                f[n][0] %= mod1;
            }
