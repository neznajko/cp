...
private:
    i64 n;
    pi64 get_edge( i64 j, i64 mask ){
        return { j >> 1, j & mask };
    }
    string bitstr( i64 j ){
        return bitset<3>( j ).to_string();
    }
public:
    Algorithm( i64 n ):
        n( n )
    ...
    void pusk() {
        ...
        w<i64> adj( m + 1 );
        For( j, 0, s ){
            auto [a,b] = get_edge( j, m );
            dbg3( bitstr( j ), bitstr( a ), bitstr( b ));
            adj[a].psh( b );
        }
        dbg( adj );
