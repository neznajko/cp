...
private:
  ...
  void rename( vector<pi32>& x ){
    dbg( x ); sort( on( x )); dbg( x );
    y.resize( siz( x ));
    i32 new_name = 1;
    y[x[0].snd] = new_name;
    For( j, 1, siz( x )){
      if( x[j].fst != x[j - 1].fst ){ ++new_name; }
      y[x[j].snd] = new_name;
    }
    dbg2( y, new_name );
    count.resize( new_name + 1 );
  }
  i32 mos( i32 a, i32 b ){
    dbg4( y, a, b, count );
    dbg2( left, ryte );
    while( ryte < b ){
      if( ++count[y[++ryte]] == 1 ){ ++distinct; }
    }
    while( ryte > b ){
      if( --count[y[ryte--]] == 0 ){ --distinct; }
    }
    while( left < a ){
      if( --count[y[left++]] == 0 ){ --distinct; }
    }
    while( left > a ){
      if( ++count[y[--left]] == 1 ){ ++distinct; }
    }
    return distinct;
  }
public:
  void pusk() {
    auto n = load_scalar<i32>(); dbg( n );
    auto m = load_scalar<i32>(); dbg( m );
    vector<pi32> x( n );
    For( j, 0, n ){
      x[j] = { load_scalar<i32>(), j };
    }
    vector<tuple<i32,i32,i32>> q( m );
    For( j, 0, m ){
      auto a = load_scalar<i32>() - 1;
      auto b = load_scalar<i32>() - 1; dbg2( a, b );
      q[j] = { a, b, j };
    }
    const i32 k = sqrt( n ); dbg( k );
    dbg( q );
    sort( on( q ), [k]( const auto& x, const auto& y ){
      const auto [ax,bx,o_o] = x;
      const auto [ay,by,O_o] = y;
      if( ax/k == ay/k ){
        if( odd( ax / k )){ // zig-zag
          return bx > by;
        }
