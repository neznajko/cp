...
public:
    ...
    void pusk() {
        ...
        u64 total = 0;
        for(; j < n; ++j ){
            auto it = lower_bound( all( posn ), MKP( x[j], j ));
            if( it == posn.begin() ){ continue; }
            auto prev = *std::prev( it );
            if( fst( prev ) == x[j] and snd( prev ) >= i ){
                total += sum( j - i );
                i = snd( prev ) + 1;
                total -= sum( j - i ); // crossbar
            }
        }
        total += sum( j - i );
        dmp( total );
