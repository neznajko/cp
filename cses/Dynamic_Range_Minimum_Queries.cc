...
////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////
inline bool ryte( i64 a ){ return a & 1; }
inline bool left( i64 b ){ return !ryte( b ); }
////////////////////////////////////////////////////////////////
class Algorithm {
private:
         i64 n;
    ...
    void update( i64 k, i64 u ){
        k += n;
        tree[ k ] = u;
        while(( k = k / 2 )){
            tree[ k ] = min( tree[ 2*k ], tree[ 2*k + 1 ]);
        }}
    i64 submarine( i64 a, i64 b ){
        a += n;
        b += n;
        i64 marine = inf;
        while( a <= b ){
            if( ryte( a )){
                marine = min( marine, tree[ a ]);
                ++a;
            }
            if( left( b )){
                marine = min( marine, tree[ b ]);
                --b;
            }
            a /= 2;
            b /= 2;
...
public:
    Algorithm( i64 n, vi64& x, v<ei64>& q ):
    n( n ), x( x ), q( q )
    {
        n = 1 << ( i64 )ceil( log2( n ));
        x.resize( n, inf );
        dbg( x );
        tree = vi64( 2 * n );
        copy( on( x ), tree.begin() + n );
        for( i64 j = 2 * n - 1; j > 1; j -= 2 ){
            tree[ j/2 ] = min( tree[ j ], tree[ j - 1 ]);
        }
        this->n = n;
        dbg( tree );
    }
    void pusk() {
        for( auto [type, a, b]: q ){
            if( type == 1 ){
                const auto k{ a - 1 }, u{ b };
                update( k, u );
            } else {
                dmp( submarine( a - 1, b - 1 ));
            }}}
};
/////////////////////////////////////////////////// /  /////////
////////////////////////////////////////////////// /  //////////
...
int main() {
    ...
    if( 0 ){
        ...
        auto x = load_vector <i64> (  n );
        auto q = load_vector <ei64> ( m );
        Algorithm( n, x, q ).pusk();
    }}
////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////
