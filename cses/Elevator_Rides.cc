...
class Algorithm {
public:
    Algorithm( const i32 n, const i32 x, const vi32& w ):
        n( n ), x( x ), w( w )
    {}
    void pusk() {
        auto varionts = 1 << n;
        auto f = v<pi32>( varionts, {n,0} ); f[0] = {1,x};
        For( s, 1, varionts ){
            For( p, 0, n ){
                if( !(s & (1 << p))) { continue; }
                const auto& prev = f[s^(1 << p)];
                pi32 next = prev;
                if( w[p] <= prev.avail ){
                    next.avail -= w[p];
                } else {
                ...
                if( f[s].rides > next.rides ){
                    f[s] = next;
                } else if( f[s].rides == next.rides ){
                    if( f[s].avail < next.avail ){
                        f[s].avail = next.avail;
                    }
                }
            }
        }
...
int main() {
    ...
    if( 0 ){
        ...
        auto w = load_vector<i32>( n );
        Algorithm( n, x, w ).pusk();
    }
}
////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////
...
// (a) 701  prme (b) 1009 prme
// (c) 1949 prme (d) 1951 prme
////////////////////////////////////////////////////////////////
// しずか 静か
////////////////////////////////////////////////////////////////
//                                          ##  ######@       @@
