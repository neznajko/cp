...
class Algorithm {
public:
    Algorithm( const pu64 n, vu64& k ):
        n( n ),
        k( k )
    {}
    void pusk() {
        sort( on( k ));
        const auto f = (n.PROD - 1)/ n.MACH + 1;
        auto l = k.front() * f;
        auto u = k.back() * f;
        while( l <= u ){
            auto m = (l + u)/ 2;
            if( tscheck_time( m ) < n.PROD ){
                l = m + 1;
            } else {
                u = m - 1;
            }
...
int main() {
    ...
    if( 0 ){
        ...
        auto n = load_scalar<pu64>();
        auto k = load_vector<u64>( n.FST );
        Algorithm( n, k ).pusk();
    }
}
////////////////////////////////////////////////////////////////
