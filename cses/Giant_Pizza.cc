...
class Algorithm {
public:
    Algorithm( i64 n, w<i64>& adj ): n( n ), adj( adj )
    {}
    void pusk() {
        dbg2( n, adj );
        // dfs -> topsort -> dfs -> scc -> coloring
        // 1) adj -> dfs -> toopsort
        // 2) adj, topsort -> dfs -> scc
        // 3) if x and -x have same scc, exit
        // 4) topsort -> coloring
        auto topsort = dfs_topsort();
        dbg( topsort );
        auto scc = dfs_scc( topsort );
        dbg( scc );
        For( u, 1, n + 1 ){
            if( scc[u] == scc[u + n] ){
                dmp( "IMPOSSIBLE" );
