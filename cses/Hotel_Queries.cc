...
private:
  ...
  void build() {
    ...
    h.resize( n );
    tree = vu32( 2*n );
    copy( on( h ), tree.begin() + n );
    // 最大セグメント木
    for( u32 j = 2*n - 1; j > 1; j -= 2 ){
      tree[j/2] = max( tree[j], tree[j - 1] );
    }}
  u32 getpos( u32 y ){
    if( y > tree[1] ){ return 0; }
    u32 j = 1;
    while( true ){
      const auto left = 2*j;
      if( left >= siz( tree )){ break; }
      const auto ryte = left + 1;
      if( y > tree[ left ]){
        j = ryte;
      } else {
        j = left;
...
public:            // n m
  void pusk() {    // h1 h2 ...... hn
    cin >> n >> m; // r1 r2 ...... rm
    h = load_vector <u32> ( n ); dbg( h );
    r = load_vector <u32> ( m ); dbg( r );
    build(); dbg( tree );
    for( auto y: r ){
      auto j = getpos( y ); cout << j << sp;
      if( j ){ dec( j, y ); }
    }}
  static u32 roundup( u32 n ){
    if(!(n & (n - 1))){ return n; }
