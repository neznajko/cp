...
public:
    ...
    void pusk() {
        ...
        auto dist = vi64( n + 1, inf ); dist[1] = 0;
        auto ways = vi64( n + 1 ); ways[1] = 1;
        auto lower = v<i64>( n + 1 );
        auto upper = v<i64>( n + 1 );
        auto pq = minpq<pi64>();
        pq.push({ 0, 1 });
        while( !pq.empty()){
            ...
            for( auto [v,w]: adj[u] ){
                ...
                if( dist[v] > dist[u] + w ){
                    ...
                    ways[v] += ways[u];
                    ways[v] %= mod1;
                    lower[v] = min( lower[v], lower[u] + 1 );
                    upper[v] = max( upper[v], upper[u] + 1 );
                }
            }            
        }
        dbg4( dist, ways, lower, upper );
        cout << dist[n] << sp << ways[n] << sp
             << lower[n] << sp << upper[n] << nl;
    }
private:
    i64 n;
    v<ei64>& e;
