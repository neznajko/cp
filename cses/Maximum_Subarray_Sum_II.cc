...
class Algorithm {
public:
    Algorithm( i64 n, i64 a, i64 b, const vi64& x ):
        n( n ),
        a( a ),
        b( b ),
        x( x )
    ...
    void pusk() {
        ...
        vi64 pref( off + n );
        For( j, off, siz( pref )){
            pref[j] = pref[j - 1] + x[j - off];
        }
        mset<i64> m( pref.begin(), pref.begin() + off );
        i64 i = 0;
        i64 j = off - 1;
        i64 sub = mini64;
        For( k, b, siz( pref )){
            sub = max( sub, pref[k] - *m.begin() );
            m.erase( m.find( pref[i++] ));
            m.insert( pref[++j] );
        }
        dmp( sub );
    }
...
int main() {
    ...
    if( 0 ){
        ...
        auto a = load_scalar<i64>();
        auto b = load_scalar<i64>();
        auto x = load_vector<i64>( n );
        Algorithm( n, a, b, x ).pusk();
    }
}
////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////
