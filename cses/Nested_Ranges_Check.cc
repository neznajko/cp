...
template <typename T>
using iset = tree<T, null_type, less<T>, rb_tree_tag,
...
public:
    ...
    void pusk() {
        ...
        sort( on( ival ), Left_Comparator() );
        i32 maxryte = ival.front().ryte;
        for( i32 i = 1; i < n; ++i ){
            auto ryte = ival[i].ryte;
            if( ryte <= maxryte ){
                bot[ ival[i].posn ] = true;
            } else {
                maxryte = ryte;
            }
        }
        sort( on( ival ), Ryte_Comparator() );
        i32 maxleft = ival.front().left;
        for( i32 i = 1; i < n; ++i ){
            auto left = ival[i].left;
            if( left <= maxleft ){
                top[ ival[i].posn ] = true;
            } else {
                maxleft = left;
            }
        }
