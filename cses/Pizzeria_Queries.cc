...
public:
  Segmentation_Fault() {}
  Segmentation_Fault( const vi64& p ){ dbg( p );
    n = siz( p );
    if( n & (n - 1)){
      n = 1 << (32 - __builtin_clz( n ));
    } dbg( n );
    tree = vi64( 2*n, inf );
    copy( on( p ), tree.begin() + n );
    for( i32 j = 2*n - 1; j > 1; j -= 2 ){
      tree[j/2] = min( tree[j-1], tree[j] );
    } dbg( tree );
  }
  static bool rytechild( u32 j ){ return j & 1; }
  static bool leftchild( u32 j ){ return !rytechild( j ); }
  i64 minq( u32 a, u32 b ){
    i64 minv = inf; // 最小 値( さいしょう あたい )
    a += n; b += n;
    while( a <= b ){
      if( rytechild( a )){ minv = min( minv, tree[a++]); }
      if( leftchild( b )){ minv = min( minv, tree[b--]); }
      a /= 2; b /= 2;
    }
    return minv;
  }
   u32 getn() const { return n; }
   i64 getv( u32 k ) const { return tree[k + n]; }
  void add( u32 k, i64 x ){
    k += n;
    tree[k] += x;
    for( k /= 2; k > 0; k /= 2 ){
...
private:
                vi64 p;
  Segmentation_Fault sigseg_exception;
  Segmentation_Fault pure_virtual_call;
  
  i64 minq( u32 k ){ // ０始まりインデックス
    i64 minv = inf;
    const auto n = sigseg_exception.getn();
    auto sigseg = sigseg_exception.minq( k, n - 1 );
    minv = min( minv, sigseg - sigseg_exception.getv( k ) + p[k]);
    auto pure_virtual = pure_virtual_call.minq( 0, k );
    minv = min( minv, pure_virtual - pure_virtual_call.getv( k ) + p[k]);
    return minv;
  }
  void update( u32 k, i64 x ){
     sigseg_exception.add( k, x - p[k]);
    pure_virtual_call.add( k, x - p[k]);
    p[k] = x;
  }
public:
  void pusk() {
               ...
               auto m = load_scalar <u32> ();
                    p = load_vector <i64> ( n );
          auto sigseg = vi64( n );
    auto pure_virtual = vi64( n );
    For( j, 0, n ){
            sigseg[j] = p[j] + j;
      pure_virtual[j] = p[j] + n - 1 - j;
    }
    sigseg_exception = Segmentation_Fault( sigseg );
    pure_virtual_call = Segmentation_Fault( pure_virtual );
    while( m-- ){
      auto type = load_scalar <u32> ();
      if( type == 1 ){
        auto k = load_scalar <u32> ();
