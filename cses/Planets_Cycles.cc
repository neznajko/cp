...
private:
    i64 succ( i64 u ){ return t[u - 1]; }
    vi64 get_src() {
        auto is_src = v<bool>( n + 1, true );
        for( auto v: t ){ is_src[v] = false; }
        vi64 src;
        For( u, 1, n + 1 ){
            if( is_src[u] ){ src.psh( u ); }
        }
        return src;
    }
    vi64 get_path( i64 u ){
    ...
    void calc_dist( vi64& path ){
        auto cross = path.back();
        auto len = siz( path ) - 1;
        auto d = len + dist[cross];
        i64 j = 0;
        for(; path[j] != cross; ++j ){
            dist[path[j]] = d--; 
        }
