...
public:
    ...
    void pusk() {
        ...
        dbg3( groups, paths, p );
        for( auto [u,k]: q ){
            cout << make_query( u, k ) << nl;
        }
    }
private:
    i64 make_query( i64 u, i64 k ){
        dbg2( u, k );
        ...
        auto b = path.back();
        if( groups[u] != groups[b] ){
            k -= (siz( path ) - 1) - p[u];
            return make_query( b, k );
        }
        auto cycle_length = (siz( path ) - 1) - p[b];
        if( p[u] < p[b] ){
            k -= p[b] - p[u];
        } else {
            k -= (siz( path ) - 1) - p[u];
        }
        return path[p[b] + (k % cycle_length)];
    }
    i64 n;
    v<i64>& t;
    v<pi64>& q;
