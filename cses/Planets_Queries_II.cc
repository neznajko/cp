...
class Algorithm {
public:
    Algorithm( i64 n, vi64& t, v<pi64>& q ):
        n(n), t(t), q(q),
        groups( vi64( n + 1 )),
        p( vi64( n + 1, -1 )),
        paths( w<i64>( 1 ))
    ...
    void pusk() {
        v<bool> is_src( n + 1, true );
        for( auto u: t ){ is_src[u] = false; }
        For( u, 1, n + 1 ){
            if( !is_src[u] ){ continue; }
            get_path( u );
        }
        For( u, 1, n + 1 ){
            if( groups[u] ){ continue; }
            get_path( u );
        }
        dbg( paths );
        umap<i64,i64> m;
...
private:
    i64 make_query( i64 u, i64 v ){ // u -> v
        i64 jumps = 0;
        while( groups[u] != groups[v] ){
            auto& path = paths[groups[u]];
            auto b = path.back();
            if( groups[u] == groups[b] ){ return -1; }
            jumps += (siz( path ) - 1) - p[u];
            u = path.back();
        }
        //
        if( p[u] <= p[v] ){
            return p[v] - p[u] + jumps;
        }
        auto& path = paths[groups[u]];
        auto b = path.back();
        if( groups[b] != groups[u] ){ return -1; }
        if( p[v] < p[b] ){            return -1; }
        return (siz( path ) - 1) - p[u] + p[v] - p[b] + jumps;
    }
    i64 n;
    v<i64>& t;
    v<pi64>& q;
...
int main() {
    ...
    if( 0 ){
        ...
        auto t = load_vector<i64>( n );
        auto q = load_vector<pi64>( m );
        Algorithm( n, t, q ).pusk();
    }
}
////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////
