...
private:
    // 0 1 2 3 4 5 6 7 8  k
    //   / / / / / / / /
    //   3 2 4 5 1 1 5 3  pref
    //   -   -   -   -    p
    //   - -     - -
    //   - - - -
    //   - - - - - - - -
    //   3 2 2 5 - - 4 3  tree[ k ] = pref[ k ] - pref[k - p(k)]
    //           4 4
    void make_tree() {
        dbg2( n, y );
        tree = vi64( n + 1 );
    ...
    // k = 2, u = 1
    // tree = 3 2 2 5 -4 -4 4 3
    void inc( u32 k, i64 u ){
        while( k <= n ){ //      2   4         8
            tree[ k ] += u; // 3 3 2 6 -4 -4 4 4
            k += p( k ); // 4 8
        }}
    void updateq( u32 a, u32 b, i64 u ){
        inc( a, u );
        inc( b + 1, -u );
    }
public:
    ...
    void pusk()
    {
        ...
        y = vi64( n + 1 ); // １始まりのインデックス
        For( j, 1, n + 1 ){ cin >> y[ j ]; }
        make_tree(); dbg( tree );
        while( m-- ){
            const auto type = load_scalar <u32> ();
            if( type == 1 ){
                auto a = load_scalar <u32> ();
                auto b = load_scalar <u32> ();
                auto u = load_scalar <i64> (); // 増やす
                updateq( a, b, u );
            } else {
