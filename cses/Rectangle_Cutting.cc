...
class Algorithm {
public:
    Algorithm( i32 a, i32 b ):
        a( a ),
        b( b )
    {}
    void pusk() {
        w<i32> f( a + 1, vi32( b + 1 ));
        For( j, 2, b + 1 ){ f[1][j] = j - 1; }
        For( i, 2, a + 1 ){ f[i][1] = i - 1; }
        For( i, 2, a + 1 ){
            For( j, 2, b + 1 ){
                if( i == j ){ continue; }
                i32 res = maxi32;
                for( i32 k = 1; k <= j / 2; ++k ){
                    res = min( res, f[i][k] + f[i][j - k] + 1 );
                }
                for( i32 k = 1; k <= i / 2; ++k ){
                    res = min( res, f[k][j] + f[i - k][j] + 1 );
                }
                f[i][j] = res;
            }
        }
