...
class Algorithm {
public:
    Algorithm( i32 n, i32 m, const v<pi32>& e ):
        n( n ), m( m ), e( e ),
        adj( w<i32>( n + 1 )), dist( vi32( n + 1, -1 )),
        prnt( vi32( n + 1 ))
    {
        for( auto [u,v]: e ){
            adj[u].psh( v );
    ...
    void pusk() {
        ...
        dbg( adj );
        For( u, 1, n + 1 ){
            if( dist[u] == -1 and explore( u )){ return; }
        }
        dmp( "IMPOSSIBLE" );
    }
private:
    ...
    bool explore( i32 u ){
        ...
        while( !deq.empty()){
            ...
            for( auto v: adj[u] ){
                if( dist[v] == -1 ){
                    deq.in( v ); dist[v] = dist[u] + 1;
                    prnt[v] = u;
                } else {
                    if( dist[u] <= dist[v] and dist[u] >= 1 ){
                        dbg2( u, v );
                        travel( u, v );
                        return true;
                    }
