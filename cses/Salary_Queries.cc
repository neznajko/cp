...
private:
  vi64 p;
  vi64 values;
  v<tuple<char,i64,i64>> qs;
  vi64 tree;
  //
  static i64 P( i64 k ){ return k & -k; }
  i64 getindex( i64 y ){
    return L( on( values ), y ) - values.begin();
  }
  i64 sum( i64 k ){
    i64 s = 0;
    while( k > 0 ){
  ...
  void add( i64 k, i64 x ){
    while( k < siz( tree )){
      tree[k] += x;
      k += P( k );
    }}
  i64 sumq( i64 i, i64 j ){
  ...
  void build() {
    ...
    values.psh( 0 );
    sort( on( values ));
    values.erase( unique( on( values )), values.end());
    dbg( values );
    vi64 count( siz( values ));
    for( auto y: p ){
      ++count[ getindex( y )];
    }
    dbg( count );
    for( i64 i = 2; i < siz( count ); ++i ){
      count[i] += count[i - 1];
    }
    dbg( count );
    tree = vi64( siz( count ));
    For( j, 1, siz( tree )){
      tree[j] = count[j] - count[j - P( j )];
    }
    dbg( tree );
  }
  void load() {
