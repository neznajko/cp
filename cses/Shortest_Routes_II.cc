...
class Algorithm {
public:
    Algorithm( i64 n, const w<i64>& e, const v<pi64>& q ):
        n( n ), e( e ), q( q ),
        dist( w<i64>( n + 1, vi64( n + 1, inf )))
    {
        for( auto& edge: e ){
            auto i = edge[0];
            auto j = edge[1];
            auto w = edge[2];
            dist[i][j] = dist[j][i] = min( w, dist[i][j] );
        }
        For( i, 1, n + 1 ){
            dist[i][i] = 0;
        }
...
int main() {
    ...
    if( 0 ){
        ...
        auto q = load_scalar<i64>();
        auto e = load_tensor<i64>( m, 3 );
        auto r = load_vector<pi64>( q );
        Algorithm( n, e, r ).pusk();
    }
}
////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////
