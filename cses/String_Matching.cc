...
class Algorithm {
private:
    static constexpr u64 A = 911382323;
    static constexpr u64 B = 972663749;
    ...
    void preprocess() {
        ...
        dbg( p );
        g = key[0];
        for( u32 k = 1; k < siz( key ); ++k ){
            g = (g*A + key[k]) % B;
        }
        dbg( g );
    }
    u64 hsub( u32 a, u32 b ){
        if( a == 0 ){ return h[b]; }
...
public:
    void pusk() {
        ...
        dbg( s ); dbg( key );
        u32 n = 0;
        for( u32 k = 0; k + siz( key ) - 1 < siz( s ); ++k ){
            auto a = k;
            auto b = k + siz( key ) - 1;
            n += ( g == hsub( a, b ));
        }
...
// Memory limit: 512 MB
// 
// Given a string and a pattern, your task is to count the number
// of positions where the pattern occurs in the string.
// 
// Input
// The first input line has a string of length n, and the second 
// input line has a pattern of length m. Both of them consist of 
// characters a–z.
//
// Output
