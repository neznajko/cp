...
private:
  ...
  void merge( u32 p, u32 l, u32 r ){
    t[p].maxpref = max( t[l].maxpref, t[l].fullsum + t[r].maxpref ); 
    t[p].maxsuff = max( t[l].maxsuff + t[r].fullsum, t[r].maxsuff );
    t[p].fullsum = t[l].fullsum + t[r].fullsum;
    t[p].maximum = max( max( t[l].maximum, t[r].maximum ),
                        t[l].maxsuff + t[r].maxpref );
  }
  Node merge( Node left, Node ryte ){
    return {
      max( left.maxpref, left.fullsum + ryte.maxpref ),
      max( left.maxsuff + ryte.fullsum, ryte.maxsuff ),
      left.fullsum + ryte.fullsum,
      max( max( left.maximum, ryte.maximum ),
                left.maxsuff + ryte.maxpref )
    };
  }
  Node query( u32 k, u32 a, u32 b, u32 x, u32 y ){
    if( y < a or x > b ){ return {}; }
    if( a <= x and y <= b ){ return t[k]; }
    u32 d = (x + y)/2;
    auto left = query( 2*k, a, b, x, d );
    auto ryte = query( 2*k + 1, a, b, d + 1, y );
    return merge( left, ryte );
  }
  i64 query( u32 a, u32 b ){
    auto rezult = query( 1, a, b, 0, n - 1 );
    if( rezult.maximum < 0 ){ return 0; }
    return rezult.maximum;
  }
public:
  void pusk() {
    n = load_scalar<u32>();
    auto m = load_scalar<u32>();
    auto h = load_vector<i64>( n ); dbg( h );
