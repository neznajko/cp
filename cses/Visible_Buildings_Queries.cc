...
ostream& operator<<( ostream& s, const Node& p ){
  return s << '{' << p.vizbl << '\'' << p.maxh << '`' << p.fyorst << '}';
}
////////////////////////////////////////////////////////////////
class Algorithm {
private:
  ...
  u32 liftoff( u32 levl, u32 k ){
    if( levl >= t[k].maxh ){ return 0; }
    if( levl <= t[k].fyorst ){
      return t[k].vizbl - (levl == t[k].fyorst);
    }
    u32 l = 2*k;
    u32 r = l + 1;
    return ( liftoff( levl, l ) +
             liftoff( max( levl, t[l].maxh ), r ));
  }
  Node merge( u32 l, u32 r ){
    return { t[l].vizbl + liftoff( t[l].maxh, r ),
             max( t[l].maxh, t[r].maxh ),
             t[l].fyorst };
  }
  void build( const vu32& h ){
  ...
  pu32 liftoff( u32 levl, u32 k, u32 a, u32 b, u32 x, u32 y ){
    if( y < a or x > b ){ return {}; }
    if( a <= x and y <= b ){
      return { liftoff( levl, k ), t[k].maxh };
    }
    const u32 d = (x + y)/2;
    auto [v1,m1] = liftoff( levl, 2*k, a, b, x, d );
    auto [v2,m2] = liftoff( max( levl, m1 ), 2*k+1, a, b, d+1, y );
...
public:
  void pusk() {
    ...
    build( h );
    while( m-- ){
      auto a = load_scalar <u32> () - 1; dbg( a );
      auto b = load_scalar <u32> () - 1; dbg( b );
      dmp( liftoff( 0, 1, a, b, 0, n - 1 ).fst );
    }}
};
/////////////////////////////////////////////////// /  /////////
////////////////////////////////////////////////// /  //////////
