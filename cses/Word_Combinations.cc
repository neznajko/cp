...
////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////
constexpr u32 ALPHAS = 26;
constexpr u32 MODULO = 1'000'000'007;

struct Node {
    array <u32,ALPHAS> next;
    bool fin;
};
...
inline u32 offset( char ch ){ return ch - 'a'; }

void initlize() {
    root.reserve( 1e6 ); // 105 MB
    root.psh({});
    f.resize( siz( s ));
}
...
void insert( const string& w ){
    u32 p = 0;
    for( auto ch: w ){
        const u32 k = offset( ch );
        Node& node = root[p];
        if( !node.next[k] ){
            node.next[k] = siz( root );
            root.psh({});
        }
        p = node.next[k];
    }
...
u32 combinat( i32 j ){
    ...
    do {
        ...
        p = root[p].next[ offset( s[j++] )];
    } while( p > 0 );

    return n; 
}
u32 dp() {
   for( i32 j = siz( s ) - 1; j >= 0; --j ){
       f[j] = combinat( j );
   }
   return f[0];
}
...
// Memory limit: 512 MB
// 
// You are given a string of length n and a dictionary 
// containing k words. In how many ways can you create the 
// string using the words?
//
// Input
// The first input line has a string containing n characters 
// between a–z. The second line has an integer k: the number of 
// words in the dictionary. Finally there are k lines describing 
// the words. Each word is unique and consists of characters a–z.
////////////////////////////////////////////////////////////////
//
// Output
